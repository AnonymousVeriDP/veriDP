...
# VeriDP CIFAR-10 + ResNet Parameter Sweep Script
# Mirrors run_parameter_sweep.sh but uses CIFAR-10 + ResNet18

OUTPUT_CSV="veridp_cifar10_resnet_parameter_sweep.csv"
...
echo "=== VeriDP CIFAR-10 + ResNet Parameter Sweep ==="
...
run_experiment() {
    ...
    # Build command
    local cmd="$EXECUTABLE $DATA_PATH --dataset=cifar10 --model=$MODEL --max-batches=$FIXED_BATCHES --clip=$clip --sigma=$sigma --lr=$lr"
    
    echo -n "Running: clip=$clip, sigma=$sigma, lr=$lr ... "
...
# ============================================
# Experiment 1: Clip norm sweep
# ============================================
echo ""
echo "=== Experiment 1: Clip Norm Sweep (CIFAR-10 + ResNet) ==="
for clip in "${CLIP_NORMS[@]}"; do
    run_experiment "$clip" 1.1 0.01
done

# ============================================
# Experiment 2: Noise multiplier sweep
# ============================================
echo ""
echo "=== Experiment 2: Noise Multiplier Sweep (CIFAR-10 + ResNet) ==="
for sigma in "${NOISE_MULTIPLIERS[@]}"; do
    run_experiment 10 "$sigma" 0.01
done
...
# ============================================
echo ""
echo "=== Experiment 3: Learning Rate Sweep (CIFAR-10 + ResNet) ==="
for lr in "${LEARNING_RATES[@]}"; do
    run_experiment 10 1.1 "$lr"
done
...
echo ""
echo "Total experiments run: $(( ${#CLIP_NORMS[@]} + ${#NOISE_MULTIPLIERS[@]} + ${#LEARNING_RATES[@]} ))"
