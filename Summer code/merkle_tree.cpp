...
// Generic field element hashing - serializes field element to bytes then hashes
__hhash_digest merkle_tree::hash_single_field_element(const F& x)
{
    __hhash_digest data[2], ret;
    memset(data, 0, sizeof(data));
    
#ifdef USE_FIELD_256
    // For 256-bit field, hash 32 bytes
    unsigned char buf[32];
    x.getBytes(buf);
    memcpy(&data[0], buf, 32);
#else
    // For 61-bit field, hash real and img
...
// Generic Merkle-Damgard construction for 4 field elements
__hhash_digest merkle_tree::hash_double_field_element_merkle_damgard(const F& x, const F& y, const F& z, const F& w, __hhash_digest prev_hash)
{
    __hhash_digest data[2], ret;
    
#ifdef USE_FIELD_256
    // For 256-bit field: each element is 32 bytes
    unsigned char buffer[64];
    x.getBytes(buffer);
    y.getBytes(buffer + 32);
    memcpy(&data[0], buffer, 32);
    
    z.getBytes(buffer);
    w.getBytes(buffer + 32);
    memcpy(&data[1], buffer, 32);
#else
    // For 61-bit field: each element is 16 bytes (real + img)
    F element[2];
    element[0] = x;
    element[1] = y;
    memcpy(&data[0], element, 2 * sizeof(F));
    element[0] = z;
    element[1] = w;
    memcpy(&data[1], element, 2 * sizeof(F));
#endif
