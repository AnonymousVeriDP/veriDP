...
# VeriDP Benchmark Script
# Runs train_veridp with different batch counts and records metrics to CSV

OUTPUT_CSV="veridp_benchmark_results.csv"
...
for batches in "${BATCH_COUNTS[@]}"; do
    ...
    start_time=$(date +%s)
    output=$($EXECUTABLE $DATA_PATH --max-batches=$batches --clip=$CLIP --sigma=$SIGMA --lr=$LR 2>&1)
    end_time=$(date +%s)
    ...
    epsilon=$(echo "$output" | grep "Final ε:" | grep -oE '[0-9]+\.[0-9]+' | head -1)
